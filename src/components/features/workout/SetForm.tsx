...
interface SetFormProps {
  set?: Set
  onSubmit: (
    setData: Omit<Set, 'id' | 'workoutExerciseId' | 'order'>
  ) => void
  onCancel: () => void
  isLoading?: boolean
}
...
export default function SetForm({
  ...
  const validate = () => {
    ...
    if (formData.isPercentageBased) {
      if (
        !formData.percentageOf1RM ||
        isNaN(percentage) ||
        percentage < 1 ||
        percentage > 200
      ) {
        newErrors.percentageOf1RM =
          'Percentage must be between 1% and 200%'
      }
    } else {
    ...
    if (
      ...
      newErrors.numSets =
        'Number of sets must be between 1 and 10'
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      const setData = {
        weight: formData.isPercentageBased
          ? null
          : parseFloat(formData.weight),
        reps: parseInt(formData.reps),
        rpe:
          formData.rpe && formData.rpe.trim() !== ''
        ...
        restTime:
          formData.restTime &&
          formData.restTime.trim() !== ''
            ? parseInt(formData.restTime)
            : null,
        isPercentageBased: formData.isPercentageBased,
        percentageOf1RM: formData.isPercentageBased
          ? parseFloat(formData.percentageOf1RM)
          : null,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex space-x-4">
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  checked={formData.isPercentageBased}
                  onChange={() =>
                    handleChange('isPercentageBased', true)
                  }
                  className="rounded border-input bg-background text-primary focus:ring-primary"
                />
                <span className="text-sm text-foreground">
                  {t('percentageOf1RM') || '% of 1RM'}
                </span>
              </label>
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={formData.rpe}
              onChange={(e) =>
                handleChange('rpe', e.target.value)
              }
              className="w-full px-3 py-2 border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-primary bg-background text-foreground placeholder:text-muted-foreground"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                handleChange('restTime', e.target.value)
              }
              className="w-full px-3 py-2 border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-primary bg-background text-foreground placeholder:text-muted-foreground"
          ...
          {!set && (
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleChange('numSets', e.target.value)
                }
                className="w-full px-3 py-2 border border-input rounded-md focus:outline-none focus:ring-2 focus:ring-primary bg-background text-foreground placeholder:text-muted-foreground"
              ...
              <p className="text-xs text-muted-foreground mt-1">
                This will create {formData.numSets || 1}{' '}
                identical set(s) with the same weight and
                reps
              </p>
            </div>
          )}
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) =>
                  handleChange('isWarmup', e.target.checked)
                }
                className="rounded border-input bg-background text-primary focus:ring-primary"
              />
              <span className="text-sm text-foreground">
