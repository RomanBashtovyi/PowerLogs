...
import { EXERCISE_CATEGORIES, MUSCLE_GROUPS, EQUIPMENT_TYPES } from '@/constants/categories'
...
export default function ExerciseForm({ exercise, onSubmit, onCancel, isLoading }: ExerciseFormProps) {
  const { t } = useLanguage()
  const router = useRouter()

  const parseMuscleGroups = (muscleGroupsString?: string): MuscleGroup[] => {
  ...
  const [formData, setFormData] = useState<ExerciseFormData>({
    ...
    muscleGroups: exercise?.muscleGroups || JSON.stringify([]),
  ...
  const handleMuscleGroupToggle = (muscleGroup: MuscleGroup) => {
    ...
    setFormData({ ...formData, muscleGroups: JSON.stringify(updated) })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onSubmit) {
        ...
        setSuccessMessage(exercise ? t('exerciseUpdatedSuccess') : t('exerciseCreatedSuccess'))
      ...
      setErrors({ submit: error instanceof Error ? error.message : t('failedToSaveExercise') })
    }
  }
    ...
    <div className="max-w-2xl mx-auto">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
            ...
            {Object.entries(MUSCLE_GROUPS).map(([key, value]) => (
              <label
                ...
                className={`flex items-center gap-2 p-3 border rounded-md cursor-pointer transition-colors ${
                  selectedMuscleGroups.includes(key as MuscleGroup)
              ...
              >
                <input
                  type="checkbox"
                  checked={selectedMuscleGroups.includes(key as MuscleGroup)}
                  onChange={() => handleMuscleGroupToggle(key as MuscleGroup)}
          ...
          {errors.muscleGroups && <p className="text-destructive text-sm mt-1">{errors.muscleGroups}</p>}
        </div>
        ...
        <div className="flex gap-3 pt-4">
          <Button type="submit" disabled={isLoading} className="flex-1">
            {isLoading ? t('loading') : exercise ? t('updateExerciseButton') : t('createExerciseButton')}
          </Button>
